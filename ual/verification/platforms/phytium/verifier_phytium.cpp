...
TeeErrorCode AttestationVerifierPhytium::Initialize(
  ...
  // Check the platform
  if (report.str_tee_platform() != kUaPlatformPhytium) {
    ELOG_ERROR("It's not %s platfrom, input platform is [%s]",
               kUaPlatformPhytium, report.str_tee_platform().c_str());
  ...
  JSON2PB(report.json_report(), &phytium_report);
  b64_report_ = phytium_report.b64_quote();
  report_.SetValue(b64_report_);
  TEE_CHECK_RETURN(report_.FromBase64().GetError());

  // Parse the attester attributes in phytium report
  TEE_CHECK_RETURN(ParseAttributes());

  // Set the platform for UnifiedAttestationAttributes
  attributes_.set_str_tee_platform(kUaPlatformPhytium);

  // Set the hex_spid empty
  attributes_.set_hex_spid("");

  // Show the attester attributes in report
  ELOG_DEBUG("Initialize phytium verifier successfully");
  TEE_CHECK_RETURN(ShowAttesterAttributes());
  return TEE_SUCCESS;
}

TeeErrorCode AttestationVerifierPhytium::VerifyPlatform(
...
TeeErrorCode AttestationVerifierPhytium::GetReportQuote(std::string* quote) {
  quote->assign(b64_report_);
  return TEE_SUCCESS;
}

/// Parse the attestation attributes fields in phytium report
TeeErrorCode AttestationVerifierPhytium::ParseAttributes() {
