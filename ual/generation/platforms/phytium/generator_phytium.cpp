...
static TeeErrorCode get_phytium_attestation_report(
    const uint8_t* user_data_buf,
    int user_data_len,
    uint8_t* report_buf,
    int* report_size) {
...
TeeErrorCode AttestationGeneratorPhytium::Initialize(
...
TeeErrorCode AttestationGeneratorPhytium::GetQuote(
  ...
  // Get the phytium report
  uint8_t report_buf[PHYTIUM_ATTESTATION_USER_DATA_SIZE];
  int report_size = PHYTIUM_ATTESTATION_USER_DATA_SIZE;
  TEE_CHECK_RETURN(get_phytium_attestation_report(report_data_buf,
      PHYTIUM_ATTESTATION_USER_DATA_SIZE, report_buf, &report_size));
...
TeeErrorCode AttestationGeneratorPhytium::CreateBgcheckReport(
    const UaReportGenerationParameters& param,
    kubetee::UnifiedAttestationReport* report) {
  // Get attestation quote
  kubetee::PhytiumReport phytium_report;
  TEE_CHECK_RETURN(GetQuote(param, phytium_report.mutable_b64_quote()));

  // Make the final report with quote only
  report->set_str_tee_platform(kUaPlatformPhytium);
  report->set_str_report_type(kUaReportTypeBgcheck);
  PB2JSON(phytium_report, report->mutable_json_report());

  return TEE_SUCCESS;
}

TeeErrorCode AttestationGeneratorPhytium::CreatePassportReport(
    const UaReportGenerationParameters& param,
    kubetee::UnifiedAttestationReport* report) {
  // Get attestation quote
  kubetee::PhytiumReport phytium_report;
...
TeeErrorCode AttestationGeneratorPhytium::VerifySubReportsTrusted(
